...
const divisibleByFiveTwoToThePower = (input) => {  // Thanks for help from: Sam Panek
  let filterArr = input.map(array => array.filter(number => number %5 === 0));
  let secondFilter = filterArr.map(array => array.filter(number => typeof(number) === 'number'));
  let newArr = [];
  for (let i = 0; i < secondFilter.length; i++) {
    let tempArr = [];
    for (let j = 0; j < secondFilter[i].length; j++) {
      if (secondFilter[i].length > 0) {
        tempArr.push(Math.pow(2, secondFilter[i][j]));
      }
    }
    newArr.push(tempArr);
  }
